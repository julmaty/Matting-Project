...
class InferenceAPI:
    ...
    def __init__(self) -> None:
        ...
        self.device = device
        self.predictor = build_sam2_video_predictor_matting(
    ...
    def add_points(
        ...
        with self.autocast_context(), self.inference_lock:
            ...
            masks_alpha = torch.sigmoid(masks[:, 0]).cpu().numpy()
            masks_alpha = (masks_alpha * 255).astype(np.uint8)

            rle_mask_list = self.__get_rle_mask_list(
                object_ids=object_ids, masks=masks_alpha
            )
    ...
    def add_mask(self, request: AddMaskRequest) -> PropagateDataResponse:
        ...
        with self.autocast_context(), self.inference_lock:
            ...
            masks_alpha = torch.sigmoid(video_res_masks[:, 0]).cpu().numpy()
            masks_alpha = (masks_alpha * 255).astype(np.uint8)

            rle_mask_list = self.__get_rle_mask_list(
                object_ids=obj_ids, masks=masks_alpha
            )
    ...
    def remove_object(self, request: RemoveObjectRequest) -> RemoveObjectResponse:
        ...
        with self.autocast_context(), self.inference_lock:
            ...
            for frame_index, video_res_masks in updated_frames:
                ...
                masks_alpha = (masks_alpha * 255).astype(np.uint8)
                rle_mask_list = self.__get_rle_mask_list(
                    object_ids=new_obj_ids, masks=masks_alpha
                )
                results.append(
                    PropagateDataResponse(
    ...
    def encode_alpha_mask(self, mask: np.ndarray) -> str:
        """Кодирует альфа-маску с значениями от 0 до 255 в формате RLE и возвращает строку."""
        rle = []
        mask_flat = mask.flatten()
        
        # Проходим по всем значениям маски и группируем одинаковые значения
        for value, group in itertools.groupby(mask_flat):
            length = len(list(group))  # Длина группы одинаковых значений
            rle.append(f"{value}:{length}")  # Сохраняем значение и длину группы в формате строки
        
        return ",".join(rle)  # Объединяем все пары в одну строку через запятую
    ...
    def __get_mask_for_object(
        ...
        """
        mask_rle = self.encode_alpha_mask(np.array(mask, dtype=np.uint8, order="F"))
